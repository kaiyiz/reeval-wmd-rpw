...
data = sio.loadmat('data/bbcsport-emd_tr_te_split.mat')
...
words = words[:, sampled_indices]  # Keep the original name 'words'

# Assuming C corresponds to labels or classes, keep it as is
C = data['C']  # Keep the original name 'C'

# Save the sampled dataset with the original names
sio.savemat('data/bbcsport-emd_tr_te_split_reduced.mat', {'TR': TR, 'TE': TE, 'X': X, 'Y': Y, 'BOW_X': BOW_X, 'words': words, 'C': C})
